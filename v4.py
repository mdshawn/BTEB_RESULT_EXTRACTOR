...
def process_directory(input_dir, output_dir):
    ...
    for filename in os.listdir(input_dir):
        if filename.lower().endswith('.pdf'):
            ...
            # Determine the semester, regulation, trade, and examination_held from non-null values
            for field in ["result_semester", "regulation", "trade", "examination_held"]:
                non_null_values = [result[field] for result in results if result[field] is not None]
                if non_null_values:
                    common_value = Counter(non_null_values).most_common(1)[0][0]
                    for result in results:
                        if result[field] is None:
                            result[field] = common_value
